...
%     dE_dm=reshape(repmat(dE_dm',nY,1),size(ddelta_c_dX,2),[])';
    ...
    c = sum(sum((E).*ce));
    [almc,dalmc,mtv] = ALM_constraint(Xg,nely,nY,np,p);
    ...
    %
    %% PRINT RESULTS
    fprintf(' It.:%5i Obj.:%4.3e Vol.:%7.3f kktnorm.:%7.3f almc.:%7.3f  mtv.:%7.3f ch.:%7.3f\n',outit,c, ...
        mean(xPhys(:)),kktnorm,almc,mtv,change);
